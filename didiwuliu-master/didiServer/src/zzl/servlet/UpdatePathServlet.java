    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            for (int i = 0, len = data.length; i < len; i++) {
                mySql.updateData("insert into path (iduser,location,destination,orderNum,carriage) values (?,?,?,?,?)"
                        , user.getUserID()
                        , data[i].getLocation()
                        , data[i].getDestination() // Assuming PathBean has a getDestination() method
                        , i
                        , data[i].getCarriage());
            }
            conn.commit();
